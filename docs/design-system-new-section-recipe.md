...
- With **`yarn build && yarn exec serve dist -l 4173`** running: **`yarn lighthouse:home`** (writes gitignored `lighthouse-home.html`; see `package.json`). **WCAG gating** stays on **`yarn test:a11y`**, not Lighthouse.
